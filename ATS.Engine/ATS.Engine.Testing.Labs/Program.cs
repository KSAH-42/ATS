...
namespace ATS.Engine.Testing.Labs
{
	using ATS.Engine.Net;
	...
	public class Client : IClient
	{
		...
		protected TResponse Invoke<TResponse>( FrontController controller )
			where TResponse: BaseResponse
		{
			return controller.Execute( new FrontControllerContext() ) as TResponse ?? throw new Exception();
		}

		protected Task<TResponse> InvokeAsync<TResponse>( FrontController controller )
			where TResponse : BaseResponse
		{
			return Task<TResponse>.Run( () => controller.Execute( new FrontControllerContext() ) as TResponse ?? throw new Exception() );
		}

		protected Task<TResponse> InvokeAsync<TResponse>( FrontController controller , CancellationToken cancellationToken )
			where TResponse : BaseResponse
		{
			return Task<TResponse>.Run( () => controller.Execute( new FrontControllerContext() ) as TResponse ?? throw new Exception() , cancellationToken );
		}
	}

	public abstract class FrontController
	{
		public abstract BaseResponse Execute( FrontControllerContext context );
	}

	public class FrontControllerContext
	{
		public TimeSpan Timeout { get; set; }
		public string Server { get; set; }
	}

	public class AuthenicationController : FrontController
	{
		private readonly AuthenticationRequest _request;

		public AuthenicationController(AuthenticationRequest request )
		{				
			_request = request;
		}

		public override BaseResponse Execute( FrontControllerContext context )
		{
			return new AuthenticationResponse( Guid.Empty , Guid.Empty );
		}
	}

	public class SaveEntityController : FrontController
	{
		private readonly SaveEntityRequest _request;

		public SaveEntityController( SaveEntityRequest request )
		{
			_request = request;
		}
		...
		public override BaseResponse Execute( FrontControllerContext context )
		{
			return new SaveEntityResponse( false );
		}
	}
